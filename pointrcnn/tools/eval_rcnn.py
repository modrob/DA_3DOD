...
parser.add_argument('--inf_mod', type=str, default='test', help='specfiy the dataset for inference')
...
parser.add_argument("--dataset", type=str, default='kitti',
                    help='specify the saved features for rcnn evaluation when using rcnn_offline mode')
parser.add_argument("--with_replace", action='store_true',
                    help='specify the saved features for rcnn evaluation when using rcnn_offline mode')
parser.add_argument("--npoints_faraway", type=int, default=4000)
parser.add_argument("--shuffle_subsample", type=str, default=None)
    ...
    for data in dataloader:
        ...
        if not args.test:
            ...
            if gt_num > 0:
                ...
                #added for sampling pseudo labels by modrob
                print("sample_id: ",sample_id, " . With gt_max_iou_in: ", gt_max_iou_in)
    ...
    for data in dataloader:
        ...
        if not args.test:
            ...
            for k in range(batch_size):
                ...
                #added by modrob
                #print("Sample_ID: ", sample_id, " .First cur_gt_boxes3d :", cur_gt_boxes3d)
                ...
                if tmp_idx >= 0:
                    ...
                    #added for sampling pseudo labels by modrob
                    #print("sample_id: ",sample_id[k], " .With cur_gt_boxes3d :", cur_gt_boxes3d ," .With gt_max_iou_in: ", gt_max_iou_in)

                    for idx, thresh in enumerate(thresh_list):
                        total_roi_recalled_bbox_list[idx] += (gt_max_iou_in > thresh).sum().item()
                        #print("sample_id: ", sample_id[k]," idx: ", idx, " item: ", gt_max_iou)
                        #print("sample_id: ", sample_id[k]," idx : ", idx, "  with sum().item(): ", (gt_max_iou_in > thresh).sum().item()) 
                if not cfg.RPN.FIXED:
        ...
        for k in range(batch_size):
            ...
            #print("sample_id: ",sample_id[k], " cur_inds: ",cur_inds)
            # NMS thresh
            # rotated nms
            boxes_bev_selected = kitti_utils.boxes3d_to_bev_torch(pred_boxes3d_selected)
            ...
            pred_boxes3d_selected = pred_boxes3d_selected[keep_idx]
            scores_selected = raw_scores_selected[keep_idx]
            #print(sample_id[k]," with scores_selcted: ", scores_selected)
            pred_boxes3d_selected, scores_selected = pred_boxes3d_selected.cpu().numpy(), scores_selected.cpu().numpy()
...
##self-made code by modrob
            #cur_gt_boxes3d = gt_boxes3d[k,cur_inds]
            #cur_gt_boxes3d = torch.from_numpy(cur_gt_boxes3d).cuda(non_blocking=True).float()
            #iou3d = iou3d_utils.boxes_iou3d_gpu(pred_boxes3d[k, cur_inds], cur_gt_boxes3d)
            #gt_max_iou, _ = iou3d.max(dim=0)


            ##added by modrob
            # calculate recall
            #cur_gt_boxes3d = gt_boxes3d[k,cur_inds.cpu().numpy()]
            #print("sample id: ",sample_id[k], " cur_gt_boxes3d: ", cur_gt_boxes3d)
            #tmp_idx = cur_gt_boxes3d.__len__() - 1
            ...
            #try:
            #    print("manipulated keep_idx: ",keep_idx.cpu().numpy())
            #except:
            #    pass

            #print("original cur_gt_boxes3d: ", cur_gt_boxes3d)
            #cur_gt_boxes3d = cur_gt_boxes3d[keep_idx]

            #print("keep cur_gt_boxes3d: ", cur_gt_boxes3d)
            ...
            #    total_gt_bbox += cur_gt_boxes3d.shape[0]
                ...
                # original recall
            #    iou3d_in = iou3d_utils.boxes_iou3d_gpu(roi_boxes3d[k,cur_inds], cur_gt_boxes3d)
                #print(iou3d_in)
            #    gt_max_iou_in, _ = iou3d_in.max(dim=0)
                #print("sample_id: ",sample_id[k], " with gt_max_iou_in: ", gt_max_iou_in)
...
def eval_single_ckpt(root_result_dir):
    if args.inf_mod == 'train':
        inf_var = cfg.TRAIN.SPLIT
    else:
        inf_var = cfg.TEST.SPLIT
    root_result_dir = os.path.join(root_result_dir, 'eval')
    # set epoch_id and output dir
    num_list = re.findall(r'\d+', args.ckpt) if args.ckpt is not None else []
    epoch_id = num_list[-1] if num_list.__len__() > 0 else 'no_number'
    root_result_dir = os.path.join(root_result_dir, 'epoch_%s' % epoch_id, inf_var)
    if args.test:
...
def repeat_eval_ckpt(root_result_dir, ckpt_dir):
    ...
    log_file = os.path.join(root_result_dir, 'log_eval_all_%s.txt' % inf_var)
    logger = create_logger(log_file)
    logger.info('**********************Start logging**********************')
    ...
    ckpt_record_file = os.path.join(root_result_dir, 'eval_list_%s.txt' % inf_var)
    with open(ckpt_record_file, 'a'):
        pass

    # tensorboard log
    tb_log = SummaryWriter(log_dir=os.path.join(root_result_dir, 'tensorboard_%s' % inf_var))
    ...
    while True:
        ...
        # start evaluation
        cur_result_dir = os.path.join(root_result_dir, 'epoch_%s' % cur_epoch_id, inf_var)
...
def create_dataloader(logger):
    ...
    DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "multi_data", args.dataset)
    print("This is the DATA_PATH :", DATA_PATH)

    # create dataloader
    test_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=inf_var, mode=mode,
                                random_select=args.random_select,
                                rcnn_eval_roi_dir=args.rcnn_eval_roi_dir,
                                rcnn_eval_feature_dir=args.rcnn_eval_feature_dir,
                                classes=cfg.CLASSES,
                                logger=logger, npoints_faraway=args.npoints_faraway)
