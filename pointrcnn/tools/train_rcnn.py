...
parser.add_argument("--self_training_textfile", type=str, default=None, help='specify the textfile with pseudo-labels that are used for training')
...
def create_dataloader(logger):
    ...
    train_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=cfg.TRAIN.SPLIT, mode='TRAIN',
                                 ...
                                 subsample=args.subsample, shuffle_subsample=args.shuffle_subsample, self_training=args.self_training, self_training_textfile=args.self_training_textfile)
...
if __name__ == "__main__":
    ...
    if args.rcnn_ckpt is not None:
        pure_model = model.module if isinstance(model, torch.nn.DataParallel) else model
        total_keys = pure_model.state_dict().keys().__len__()
        train_utils.load_rcnn_part_ckpt(pure_model, filename=args.rcnn_ckpt, logger=logger, total_keys=total_keys)
